...
def download_model():
    subprocess.run(
        ...
        env={
            **os.environ,
            "HUGGING_FACE_HUB_TOKEN": os.environ["HF_READ_TOKEN"],
        },
        check=True,
    )
...
#
# For this step to work on a gated model such as LLaMA 2, the HF_READ_TOKEN environment
...
tgi_image = (
    Image.from_registry("ghcr.io/huggingface/text-generation-inference:1.3")
    .dockerfile_commands("ENTRYPOINT []")
    .run_function(download_model, secret=Secret.from_name("huggingface-secrets"))
...
@stub.cls(
    secret=Secret.from_name("huggingface-secrets"),
    gpu=GPU_CONFIG,
...
def main():
    print(Model().generate.remote("Implement a Python function to compute the Fibonacci numbers."))
...
# $ python
# >>> import modal
# >>> f = modal.Function.lookup("example-tgi-Llama-2-7b-chat-hf", "Model.generate")
